...
# for new formulation
class AntV3():
    def __init__(self, id, network: Network, sfc: SFC, cfg, keypoint_consume=None):
        self.id = id
        self.network = network
        self.sfc = sfc
        self.cfg = cfg
        # self.keypoint_consume = keypoint_consume
        ...
        self.current_id = self.sfc.path[-1]
        self.path = self.sfc.path
        self.vnf_location = []
        self.vnf_demand = copy.copy(self.sfc.vnf_demand)
        self.finished = False
        self.total_installed_vnf = len(self.sfc.vnf_location)

    def run(self):
        self.finished = False

        if self.sfc.finished == True:
            self.finished = True
            return
        
        current_node = self.network.N[self.current_id]
        if len(self.path) == 1: # starting point
        ...
        if len(self.vnf_demand) > 0:
            if self.debug:
                print("="*20)
                print(f"SFC: {self.source_id}->{self.destination_id} | Ant {self.id} start from {self.current_id} ...")
            
            if current_node.vnf_used:
                current_vnf = self.vnf_demand[0]
                if current_vnf in current_node.vnf_used:
            ...
            if next_subpath:
                self.current_id = next_subpath[-1]
            else:
                return
            ...
            if current_node.type == 1:
                while(True):
                    ...
                    if state is False: break
            self.finished = True
        elif self.current_id != self.destination_id:
            next_subpath = self.subpath_selection(node=current_node)
            if next_subpath:
                self.finished = True
                self.current_id = next_subpath[-1]
                if(self.current_id != self.destination_id):
                    print("WRONGGGGGG")
            else:
                return
        else:
        ...
        if self.finished:
            self.update_sfc()
        # self.total_installed_vnf += len(self.vnf_location)
        if self.debug: print("Final path: ", self.path)

    def update_sfc(self):
        if len(self.vnf_demand) == 0 and self.current_id == self.destination_id:
            self.sfc.path = self.path
            self.sfc.vnf_location = self.vnf_location
            self.sfc.vnf_demand = self.vnf_demand
    ...
    def vnf_selection(self, node: Node):
        ...
        if self.cfg.use_sfc_pheromone:
            pheromones = np.array([self.network.pheromone[self.sfc.id][server.id][server.id] for server in candidate_servers])
        else:
    ...
    def subpath_selection(self, node: Node):
        ...
        for subpath in subpaths:
            ...
            # subpaths_src.append(np.mean(np.array([self.network.N[subpath[i]].ratio_performance() for i in range(1, len(subpath))]))) #server_consume1
            ...
            if self.cfg.use_sfc_pheromone:
                subpaths_pheromone.append(np.mean(np.array([self.network.pheromone[self.sfc.id][subpath[i]][subpath[i+1]] for i in range(len(subpath) - 1)])))
            else:
    ...
    def generate_candidate_subpath(self, node: Node, candidate_servers_id, k_paths_per_candidate=1):
        ...
        for next_id in candidate_servers_id:
            ...
            for cnt, path in enumerate(networkx.shortest_simple_paths(self.nx_network, node.id, next_id, weight='delay')):
    ...
    def __repr__(self) -> str:
        return f'Ant {self.id} | SFC {self.sfc.id}, {self.sfc.source}->{self.sfc.destination} | Route: {self.path} | VNF location: {self.vnf_location} | Finished: {self.finished}'