    ...
    def run(self, cfg):
        ...
        while(True):
            ...
            for iter_loop in pbar:
                self.swarm = [SubSwarmAnt(i, self.network, self.sfc_set, cfg) for i in range(cfg.num_families)]
                ...
                for rank, subswarm in enumerate(best_subswarms):
                    ...
                    if cfg.use_weight_pheromone:
                        weight = subswarm.fitness / global_best_subswarm.fitness if global_best_subswarm is not None else 1
            ...
            # update sfc
            for ant in global_best_subswarm.ants:
                ant.update_sfc()
            ...
            if cfg.log_path:
                ...
                with open(f"{cfg.log_path}/status.txt", "a") as f:
                    f.write(info)
            if (cfg.display): 
                print(f"After step {step}, number of active server: {global_best_subswarm.num_actived_servers}, best score: {global_best_subswarm.objective_value}({global_best_subswarm.R_cap},{global_best_subswarm.R_server},{global_best_subswarm.R_vnf})")
                print("="*20)

            # check stopping criter_loopia
